$(document).ready(function () {
	...
	function initializeFirstSlider() {
		if ($('#filter__years').length) {
			...
			var firstValuesInput = [
				document.getElementById('filter__years-min'),
				document.getElementById('filter__years-max')
			]
			firstSlider.noUiSlider.on('update', function( values, handle ) {
				firstValues[handle].innerHTML = values[handle];
				firstValuesInput[handle].value = values[handle];
			});
		} else {
			return false;
		}
	...
	function initializeSecondSlider() {
		if ($('#filter__imbd').length) {
			...
			var secondValuesInput = [
				document.getElementById('filter__imbd-min'),
				document.getElementById('filter__imbd-max')
			];

			secondSlider.noUiSlider.on('update', function( values, handle ) {
				secondValues[handle].innerHTML = values[handle];
				secondValuesInput[handle].value = values[handle];
			});
	...
	function initializeThirdSlider() {
		if ($('#slider__rating').length) {
			...
			var thirdValueInput = document.getElementById('form__rating-value');

			thirdSlider.noUiSlider.on('update', function( values, handle ) {
				thirdValue.innerHTML = values[handle];
				thirdValueInput.value = values[handle];
			});
		} else {
			return false;
		}
